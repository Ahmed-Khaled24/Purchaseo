import { validateBirthDate, validateEmail, validatePassword, validatePhoneNumber, validateRole, validateSocialType, validateUserName, validateUserType } from "../../util/user.validation";
...
describe("test role validation", () => {
    ...
    test("test role with invalid role", () => {
        ...
        expect(() => {
            validateRole(role);
        }).toThrowError("role must be either Customer, Seller, Company or Admin");
    });
});
...
describe("test social_type validation", () => {
    ...
    test("test social_type with invalid social_type", () => {
        ...
        expect(() => 
        validateSocialType(social_type)
        ).toThrowError("socialType must be either google or local");
    });
});
