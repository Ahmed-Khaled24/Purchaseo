...
async function addNewOrder_company(order: any) {
	const { order_id, consumer_id, product_id, contract_id, time, quantity } =
		order;
	const [rows, fields] = await dbConnection.execute(
		'INSERT INTO `company_order` (`order_id`, `consumer_id`, `product_id`, `contract_id`, `time`, `quantity`) VALUES (?,?,?,?,?,?)',
		[order_id, consumer_id, product_id, contract_id, time, quantity]
	);
	const ResultRows = rows as RowDataPacket[];
	return ResultRows ? true : false;
}
